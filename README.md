...
- 根目录默认离线测试在 Plan 01-04 persistence codec 合并后为 `315 passed`。
...
`W2-CONTRACT-FREEZE` 已通过 PR #9 合并；GSD activation 已由 [PR #10](https://github.com/weijie567/mini-agent/pull/10) 生效。Plan 01-01 的 planning-status [PR #11](https://github.com/weijie567/mini-agent/pull/11) / Project Direction owner [PR #12](https://github.com/weijie567/mini-agent/pull/12)，Plan 01-02 的 planning-status [PR #13](https://github.com/weijie567/mini-agent/pull/13) / Memory owner [PR #14](https://github.com/weijie567/mini-agent/pull/14)，Plan 01-03 的 planning-status [PR #15](https://github.com/weijie567/mini-agent/pull/15) / Thin Slice mapping [PR #16](https://github.com/weijie567/mini-agent/pull/16) / projection clarification [PR #17](https://github.com/weijie567/mini-agent/pull/17)，以及 Plan 01-04 planning [PR #18](https://github.com/weijie567/mini-agent/pull/18) / persistence codec [PR #19](https://github.com/weijie567/mini-agent/pull/19) 已依次合并；当前 integration exact head 为 `bde99edec0bbb9ba331c6099c8b467c14fe24e58`。01-04 在 exact two-file allowlist 内实现 17-item registry、66 条 top-level / 7 条 logical-child projection 与 strict logical codec，并通过 134 个 focused / 315 个 full tests、两路 final exact-head review与 Graphify code + semantic freshness gate；它仍不证明 Adapter、table、migration、Runtime、HTTP、complete recovery graph 或 Eval Harness 已实现。规划审查已确认 frozen Port 缺少 codec 所需 external relation context、initial/transition/Run-finalization原子 aggregate，并且不能表达 Memory 15.2 的 fenced complete-graph recovery claim；因此插入 dependency Packet `01-04D` 单写 Application Port / Command owner contract。其合并前，原 Plans 01-05 Runtime、01-06 Infra、01-07 Eval保持 `BLOCKED`；01-08仍由 Integrator串行集成。实时状态与证据见[多 Agent 实施计划](docs/implementation/e2e01-thin-slice-multi-agent-plan.md)。
