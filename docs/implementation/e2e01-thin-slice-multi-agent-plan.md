...
- `$gsd-code-review` 只在 exact-integration-SHA review-artifact Worktree 中以规范化绝对路径的 exact `--files` 运行；preflight 必须证明 requested=accepted、每项均为仓库内 tracked file，workflow transcript 必须报告相同精确数量且不含 stock 的 outside-repository / file-not-found skip 输出，唯一写入为 Phase `REVIEW.md`。
...
| GSD | `BLOCKED_REVIEW_SCOPE_REMEDIATION / PAUSED / NOT_EFFECTIVE` | activation branch 从 integration exact base `85eb2a7...` fork；`1e6999c...`、`f740812...` 与 `b659c33...` 均保留 blocked-review 记录；`b659c33...` compatibility review 为 `PASS`、owner review 因不存在的 path-skip token 为 `BLOCK`；新候选 head 的双 review 与 PR merge 为 `PENDING` |
