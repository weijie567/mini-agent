...
| `fx-retry-scheduled-obsolete-run-owner-a-v1` | exact obsolete Run/Task/ToolCall attempt-1 `RETRY_SCHEDULED` closure已持久化，但同 Task/binding已推进到新版本；同一fixture还必须保存一个identity不同、started_at不早于obsolete Run、绑定不变current Task version且result version为`null`的authoritative v2 replacement Run/RunTaskLink。recovery只从该typed setup provenance取得`replacement_run_id`并得到`STATE_OR_BINDING_INVALIDATED`，不得创建新RU、Message、Binding或Task/RequestUnit mutation |
...
| `fault:get-shipment:restart-after-retry-finalize-v1` | 同一ordered fault plan先在attempt 1 `BEFORE_DISPATCH`注入`SYSTEM_FAILURE / SHIPMENT_SERVICE_TRANSIENT`并持久化`RETRY_SCHEDULED`，再于同一attempt的`AFTER_RETRY_FINALIZE`、attempt 2 fence前停止当前process；新recovery service revalidate PASS并只追加一次attempt 2 |
...
artifact 修正后的 exact step cardinality 冻结为：26 个 `REQUEST_UNDERSTANDING`、37 个
`CONTROL_CANDIDATE`（分布在 26 个 script）与 7 个 `FAULT_DIRECTIVE`。
`T2-retry-finalize-before-second-fence-state-invalidated` 是唯一只有 fault、没有新RU或
control的recovery-only script；其历史RU/Binding evidence已经存在于preseed obsolete
root。`T2-retry-unfinished-attempt-restart-blocked` 有RU、一个post-order control和fault，
没有terminal control。`logistics-required-uses-shipment`与下列10个本来已有terminal
control的普通logistics script各有两个control；不得把“每个script一个RU/一个control”
写入代码，也不得在test adapter末尾静默drain。

ordinary `order_id`-only continuation 的RU只持久化fresh accepted Claim，不携带Intent、
capability或NextMove/tool authority，因此仍必须先由现有确定性路由执行`get_order`；
成功保存OrderObservation后，再在`PROPOSE_POST_ORDER`边界消费既有
`CONTROL_CANDIDATE / PROPOSE_GET_SHIPMENT`。该步骤精确加入以下11个script：
...
`T2-assessment-delayed-boundary`、`T2-assessment-claim-corrected`、
`T2-timeout-after-dispatch-then-success`、
`T2-retry-finalize-before-second-fence-recovery`、
`T2-retry-unfinished-attempt-restart-blocked`与
`T2-two-active-packages-integrity-blocked`。除unfinished-attempt脚本外，其余10个新增
post-order control必须位于RU之后、原fault或terminal control之前；unfinished-attempt
则为RU→post-order control→fault。Case ID、fixture、expectation、message关键词或
script behavior都不得成为routing authority。

已有current ShipmentObservation的`stale-refresh-success`与
`refresh-returns-already-stale-blocked`、以及本轮有current DNR双Binding的
`assessment-delivered-not-received-current-claim`继续直接进入`get_shipment`，不得
增加冗余`get_order`或post-order control。
...
state-invalidated recovery-only script不得调用`REQUEST_UNDERSTANDING` provider；它只在
已持久化attempt-1 `RETRY_SCHEDULED`之后的真实`AFTER_RETRY_FINALIZE`边界消费并认证唯一
`FAULT_DIRECTIVE`，然后以typed `recovery_subject_run_id`和fixture中已保存的authoritative
replacement Run调用既有recovery service。最终Eval subject仍是obsolete root；不得把
replacement Run、Case ID、fixture名称或script expectation伪装成新业务authority。

unfinished-attempt restart在真实attempt start已持久化后，必须由同一个v2 CAS关闭：
ToolCall=`INTERRUPTED`及recovery decision child、AgentRun=`INCOMPLETE /
PROCESS_RESTART_DETECTED`、active Task/RequestUnit各推进一次到`BLOCKED`、RunTaskLink保存
terminal result version，并追加`RunStopped(BLOCKED)`、`TaskStateChanged`与
`ToolCallInterrupted` Trace。不得补造attempt terminal、AgentRunResult、ASSISTANT Message
或`ResponseRendered`。普通retry-finalize restart只在durable finalize后切换到既有
recovery service并追加一次attempt 2，不得合成结果或创建第二个ToolCall identity。
...
代替其未观察到的完成结果。
`REQ_TOOL(get_order,...,SUCCEEDED,FOUND)`只能由同一exact evidence closure中的
`OrderObservation`证明：Observation的`source_tool_call_id`必须等于该get_order
ToolCall identity，且`observation_id`必须等于ToolCall的`result_ref`；owner、Task、
RequestUnit与source-provenance closure仍须完整闭合。Observation缺失、重复、ref不等、
wrong-owner / wrong-graph或source mismatch时，`result_code`必须为`INVALID`并使predicate
失败；不得只凭ToolCall terminal status、attempt success、Case expectation、fixture或
最终回复推断`FOUND`。该规则同样约束既有get_order predicate，不为本次11个Case建立
特殊分支。
...
| `T2-retry-finalize-before-second-fence-recovery` | `MSG-LOGISTICS；[fx-verified-order-target-o1001-owner-a-v1]；[fx-shipment-current-owner-a-v1]；fault:get-shipment:restart-after-retry-finalize-v1` | `REQ_BINDING(order_id,$ORDER_BINDING_REF,$TASK_VERSION_AT_GATE), REQ_TOOL(get_order,1,1,SUCCEEDED,FOUND), REQ_ATTEMPT(get_order,1,SUCCESS,NONE,NONE,NOT_APPLICABLE), REQ_ATTEMPT(get_shipment,1,SYSTEM_FAILURE,SHIPMENT_SERVICE_TRANSIENT,NONE,RETRY_SCHEDULED), REQ_RECOVERY(get_shipment,1,PASS,RETRY_CONDITIONS_REVALIDATED,APPEND_ATTEMPT_2), REQ_ATTEMPT(get_shipment,2,SUCCESS,NONE,NONE,NOT_APPLICABLE), REQ_TOOL(get_shipment,1,2,SUCCEEDED,FOUND), REQ_OBSERVATION(SHIPMENT,$SHIPMENT_OBSERVATION_REF,$SHIPMENT_SOURCE_VERSION,FRESH), REQ_ASSESSMENT(NORMAL,shipment-assessment-rules.p0.v1,$SHIPMENT_OBSERVATION_REF), REQ_STOP(COMPLETED,GOAL_COMPLETED)` | `FORBID_SECOND_TOOLCALL_IDENTITY_FOR_SAME_SEMANTICS, FORBID_ATTEMPT_OVER_BUDGET, FORBID_LOSS_OF_PRIOR_ATTEMPT_EVIDENCE, FORBID_MODEL_GENERATED_FACT_OR_RESULT` | `RECOVERY_CAS_UNIQUE, ATTEMPT2_FENCE_APPENDED_ONCE / NO_RECOVERY_METADATA_DISCLOSURE` | `CF-10/12/13/14` |
...
| `T2-retry-unfinished-attempt-restart-blocked` | `MSG-LOGISTICS；[fx-verified-order-target-o1001-owner-a-v1]；[fx-shipment-current-owner-a-v1]；fault:get-shipment:restart-with-unfinished-attempt-v1` | `REQ_BINDING(order_id,$ORDER_BINDING_REF,$TASK_VERSION_AT_GATE), REQ_TOOL(get_order,1,1,SUCCEEDED,FOUND), REQ_ATTEMPT(get_order,1,SUCCESS,NONE,NONE,NOT_APPLICABLE), REQ_UNFINISHED_ATTEMPT(get_shipment,1), REQ_TOOL(get_shipment,1,1,INTERRUPTED,NONE), REQ_RECOVERY(get_shipment,1,NOT_APPLICABLE,PROCESS_RESTART_DETECTED,INTERRUPT_NO_REDISPATCH), REQ_STOP(BLOCKED,PROCESS_RESTART_DETECTED)` | `FORBID_ATTEMPT_OVER_BUDGET, FORBID_SHIPMENT_OBSERVATION, FORBID_ASSESSMENT, FORBID_MODEL_GENERATED_FACT_OR_RESULT` | `RUN_INCOMPLETE, ACTIVE_TASK_BLOCKED, NO_AGENT_RUN_RESULT, UNFINISHED_ATTEMPT_NOT_FABRICATED / NO_RETROACTIVE_REPLY` | `CF-10/12/14` |
