...
- 当前 immediate gate：01-07C Plan已经签发；本planning PR reviewed merge后先以独立Project Direction owner PR移除或对齐仍为16的易漂移signed-Plan计数。该owner PR reviewed merge前，`01-07G` planning与C/G feature dispatch均blocked；对齐后01-07G才以独立planning PR签发与01-07C相同的execution base `3f0753f7...`。01-07B已经关闭完整Case/Script、output-side semantic `case_id`答案暴露与variant-scoped安全因果Trace阻断，但不拥有外部`ModelProvider`异常合同；invalid-RU `ValidationError` / `ProviderProtocolError`分类缺口仍由后续01-07I/J/L分别按Application Port、Runtime、Eval-owned Provider consumer边界关闭。后续按`{01-07C, 01-07G} → {01-07D, 01-07H} → {01-07E, 01-07F} → 01-07I → 01-07J → {01-07K, 01-07L} → 01-08 → 01-08A`逐级签发；每组必须全部串行合并形成共同exact barrier后才进入下一组。当前证据仍不证明真实HTTP Trajectory/E2E、credentialed Qwen baseline或产品完成。
...
| 01-07G Thin Slice `get_order` source-version ruling | 先由独立Project Direction owner PR移除或对齐易漂移signed-Plan计数；随后scoped owner定义server-private version唯一来源/算法、FOUND必填、Observation/Manifest exact copy与禁止schema fallback；不实现Core DTO | `BLOCKED_BY_PROJECT_DIRECTION_ALIGNMENT / PLAN_NOT_YET_ISSUED` |
