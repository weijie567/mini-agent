...
must_haves:
  truths:
    - "01-07Z 只增加两个互斥、exact-v2 Application conditional write contract：合法zero/all-REJECT no-task closure，以及exact-one accepted initial Task graph。"
    - "两个command都必须携带可信owner scope与exact Conversation / USER Message / active Run roots；caller不能省略、union、alias、fallback或用record payload自证owner。"
    - "No-task command只能携带RU-v2 parent且没有ACCEPT、accepted child、Task effect或NextMove audit；initial-graph command只能携带一个ACCEPT child及与之bijective的InputBinding、Task、RequestUnit和links。"
...
<objective>
以TDD RED→GREEN增加两个显式、互斥的RU-v2 conditional write contract：一个只保存合法no-task Request Understanding closure；一个保存exact-one accepted Request Understanding child及完整initial Task graph。两者都在Application边界锁定trusted owner roots、closed graph与zero-partial-write返回语义。
...
- record全部actual Candidate已有exact keyed decision，且恰有一个`ACCEPT`；
...
- Message必须是该Conversation中的exact `USER` Message；
...
- parent/child/InputBinding/Task/RequestUnit使用Y产生的closed projection；command不得从一个对象重建另一个，也不得接受v1 record/child。
...
- exact field/type/signature、strict/frozen/extra-forbid、owner roots、no-task/accepted closure、Task/InputBinding/link bijection、tamper、no-union/no-bypass与v1 compatibility matrix。
- four-file changed-files、commit-order、no-merge、no-Core/Infra/Runtime change、clean Worktree。
...
1. RED `test(01-07Z): require ru v2 write contracts`只改两个owned Component tests。冻结三个command exact fields/cardinality/model config与两个Port exact signatures/docs；覆盖zero/all-reject、one-accept、wrong owner/root/message/run、missing/extra child、Task/binding/link/version/time tamper、v1 type rejection、no-union/no-bypass与existing v1 protection。Source blobs必须仍为base值。
2. GREEN `feat(01-07Z): add ru v2 write contracts`只改`records.py`和`ports.py`。增加三个exact command、closed validators、two explicit Protocol methods与bounded docs；不改existing v1 definitions/signatures，不实现Adapter，不importY result，不增加external dependency。
...
- initial-graph command只能表达一个ACCEPT、一个child、一个binding、一个Task/RequestUnit和exact links；owner/root/version/time闭合。
...
- focused/persistence/Runtime/Alembic/full、four-file containment、feature/latest-overlay独立review全部通过。
...
security_impact: `YES / TRUSTED OWNER AND ATOMIC GRAPH PRECONDITIONS` — 两条route都绑定trusted owner与exact Conversation/Message/Run；accepted route关闭Task/InputBinding/link图；no-task route不能偷偷写Task；non-APPLIED必须由AA实现zero writes。
...
handoff_format: repository/remote/branch/worktree、exact B_Q base/tree、Plan merge/blob、four base/head blobs、RED/GREEN/fix SHAs与输出、focused/persistence/Runtime/Alembic/full结果、command field/cardinality/owner/tamper/Port signature/v1 protection矩阵、changed-files/commit containment、cross-file scan、contract/security/Eval nonclaims、feature/overlay review、PR/merge SHA、与Y串行merge后B_YZ tree、风险与rollback。
</packet_contract>

<cross_file_impact>

- `CONFIRMED`：Intent / Memory / Thin Slice owner已批准zero/all-reject durable closure、one accepted Task effect、trusted owner root与atomic conditional write；Z只冻结Application boundary，无需修改canonical docs。
...
|---|---|---|---|---|
| `Z-S01` | Spoofing | record payload → owner/root authority | `MITIGATE / BLOCK` | TrustedOwnerScope + exact Conversation/USER Message/RUNNING Run；payload不能自证owner |
...
| `Z-T02` | Tampering | accepted parent/child → missing/extra/wrong Task effect | `MITIGATE / BLOCK` | one ACCEPT/child/binding/Task/Unit exact bijection与base-null/result-1 |
...
| `Z-D01` | Denial of Service | graph cardinality expansion → unbounded write | `MITIGATE / BLOCK` | exact one accepted/binding/Task graph；no-task exact zero family |
...
<task type="auto" tdd="true">
  ...
  <action>只改两个owned tests。增加三个command import/field/config/cardinality与two Protocol method exact signature/doc tests；构造canonical no-task、all-reject和one-accepted fixtures，逐项tamper owner/root/message/run/decision/child/task/binding/link/version/time；证明v1 model不能进入v2 command、v2 model不能进入v1 command，且没有union/optional/alias/bypass method。保持existing v1 tests与test fixture边界，不用DB/network/skip/xfail。</action>
  <verify>
  ...
  <done>测试精确冻结no-task与accepted graph两条互斥shape、trusted roots和Port signatures。</done>
</task>
...
<task type="auto" tdd="true">
  ...
  <action>只改两个owned Application source。增加三个strict/frozen command及closed validators；ports显式import并声明两个exact async methods。复用现有TrustedOwnerScope、Conversation/Message/Run、CreateTask/RequestUnit/InputBinding/link commands与ConditionalWriteResult。不得实现I/O、修改v1 symbols、增加union/default/fallback/hasattr probing或导入Y result。</action>
  <verify>
...
uv run pytest tests/component/application/test_persistence_contract.py tests/component/application/test_agent_run_service.py -q</automated>
  ...
  <done>Application公开两个可由AA静态实现、由J显式调用的v2 conditional write contract；v1 surface保持。</done>
</task>
...
scan只报告canonical owner、existing v1 caller/adapter、后续AA/J/W consumer与已知derived status drift；Z writer不得越allowlist修正。Feature exact head必须对四项changed files取得独立`PASS / 0/0/0/0` review。Integrator串行merge时若Z是Y之后合入者，必须在latest integration上做patch-identity/no-conflict overlay、重复focused/full/containment与review；只有Y/Z都reviewed merged后才命名B_YZ。
...
2. no-task route只有owner-bound RU-v2 parent和zero/all-REJECT closure；无Task family或NextMove audit。
3. accepted route恰好一个parent/child/binding/Task/Unit及exact links，owner/message/run/version/time闭合。
4. RuntimeRecordPort两方法exact、互斥、无union/optional/alias/fallback/bypass；v1 surface保持。
5. focused、persistence/Runtime neighbor、Alembic/full、containment、feature/latest-overlay exact-head review全部通过。
