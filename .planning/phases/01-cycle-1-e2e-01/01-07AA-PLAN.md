...
must_haves:
  ...
  key_links:
    ...
    - "Execution map r2：exact B_YZ → 01-07AA → B_J_READY；01-07J只能从reviewed B_J_READY启动。"
---
...
Purpose: 关闭`B_YZ`中“Application已经有exact-v2 command/Port，但PostgreSQL Adapter仍只有legacy-v1 initial graph writer”的最后一道01-07J前置阻断；让后续Runtime可以显式调用v2 route，而不通过active/default/latest、v1/v2 union、payload inference或legacy helper fallback持久化。

Output: 一个只增加owned Integration test的RED commit，以及一个只修改`postgres.py`的GREEN commit；review finding只用append-only fix。AA不创建Summary、不修改shared State、canonical docs、Application/Core/Runtime/Eval、migration或shared test bootstrap。
</objective>
...
remote: `origin`
head_branch: `codex/e2e01-01-ru-v2-atomic-writer`
base_branch: `integration/e2e01-thin`
base_sha: `d704b87480f0a4252744f4c009cef9a86c08fa05`
base_tree: `1162ba009147c5c8cefb8a0c2bc39254b96facef`
input_barrier: `B_YZ`
...
- exact feature base/tree `B_YZ = d704b87480f0a4252744f4c009cef9a86c08fa05` / `1162ba009147c5c8cefb8a0c2bc39254b96facef`
...
- official 01-07AA Plan merge SHA/blob由Integrator在Plan PR reviewed merge后捕获；Plan merge不替换feature exact base B_YZ
...
- `docs/implementation/e2e01-thin-slice-multi-agent-plan.md` reviewed r2 `B_YZ → 01-07AA → B_J_READY`、two-file ownership与barrier nonclaims。
- 01-07Y reviewed Core reducer、01-07Z reviewed exact-v2 commands/Ports、01-07K reviewed exact-v2 owner-scoped reader和B_YZ现有PostgreSQL adapter。
...
- report exact branch/head/base and linear commit list；
- report actual changed files and `git diff --name-only B_YZ...HEAD` exact equality to two-file allowlist；
...
<task type="auto" tdd="true">
  <name>Task 1: RED — freeze two exact-v2 PostgreSQL writer oracles</name>
  <files>tests/integration/test_postgres_v2_request_understanding_writes.py</files>
  <action>只增加新owned Integration test。复用现有public fixtures/builders而不修改shared bootstrap；构造Z exact commands，覆盖Section 1–6的first-write、01-07K round-trip、exact replay、root/owner/version/closure/CAS conflicts、v1 collision、no-task/initial互斥、fault injection、bounded errors和deterministic concurrency。增加static source assertions或monkeypatch tripwires证明v2 methods不调用legacy encoder/persist/decode/projection/physical-validation helpers。提交subject exact为`test(01-07AA): freeze ru v2 postgres writers`。</action>
  <verify>
    <automated>uv run pytest tests/integration/test_postgres_v2_request_understanding_writes.py -q</automated>
    RED必须非零且只因两个Port methods或writer-private exact-version behavior尚未实现；`postgres.py` blob仍等于B_YZ。
  </verify>
...
1. Preflight精确证明feature branch/worktree从`B_YZ=d704b874...`创建；Plan PR merge只作为issuance evidence，不替换feature base。
2. RED commit只改new Integration test，失败只因AA missing writer surface/behavior；GREEN及fix保持two-file allowlist。
