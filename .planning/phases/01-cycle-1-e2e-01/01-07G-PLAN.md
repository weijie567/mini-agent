...
must_haves:
  truths:
    ...
    - "最终 P0 语义中，只有同时携带安全 order_summary 与 non-empty exact source_version 的结果才可被接受为 FOUND 并创建 Observation / Manifest #2；NOT_FOUND_OR_NOT_ACCESSIBLE 和 SYSTEM_FAILURE 均不得携带 source_version。"
    ...
    - "合同按 expand→runtime-enforce→producer→core-contract 四阶段落地：01-07H 只增加可选表示能力，01-07J 对缺失/损坏版本 fail closed，01-07K 使 PostgreSQL producer 始终生成版本，01-07M 最后收紧 Core FOUND validator；每个中间 Packet 都必须独立通过 full suite。"
  ...
  key_links:
    - "01-07H additive-expand Core/Order DTO；01-07J在Runtime acceptance boundary fail closed并删除schema fallback；01-07K在同一次owner-scoped PostgreSQL读取上实现算法；01-07M在K/L共同barrier后收紧Core FOUND validator。"
...
- 01-08 remains blocked until the later 01-07M Core contract-closure Packet has reviewed-merged after the 01-07K/01-07L common barrier
...
- final accepted-FOUND matrix requires both `order_summary` and non-empty exact-pattern `source_version`, forbids `failure_code`, and creates Observation plus Manifest #2
...
- 01-07H is an additive expand Packet: `GetOrderResult.source_version` is optional during migration, validates the exact pattern whenever present, and remains forbidden for every non-FOUND outcome; H must not yet reject legacy `FOUND + None`, and must not claim the final matrix is complete
- 01-07H also migrates in-repo FOUND stubs/fixtures to explicit valid test tokens where their ownership allowlist permits, while the physical PostgreSQL producer remains the one declared legacy absence until 01-07K; no test token becomes runtime authority
- 01-07J is the deterministic semantic acceptance gate before any Observation/Manifest creation: a FOUND candidate with missing, empty, malformed or otherwise unusable `source_version` becomes the existing bounded SYSTEM_FAILURE path with no Observation, no Manifest #2, no retry and no fallback
- 01-07K makes the sole PostgreSQL producer return the deterministic version from the same owner-scoped read; it cannot relax 01-07J or perform a second query
- 01-07M is a new Core contract-closure Packet after the 01-07K/01-07L common barrier: it makes non-empty exact-pattern `source_version` mandatory for `GetOrderResult.FOUND`, retains the non-FOUND prohibition, runs the full suite and blocks 01-08 until reviewed merge
- the temporary H expand state is not an executable P0 completion claim and cannot reach Presentation after J; it exists only so H, J, K and M each have a green, ownership-isolated migration step
...
- generic persisted `OrderObservation.source_version?` remains optional outside this scoped FOUND producer/consumer path; this does not keep final `GetOrderResult.FOUND.source_version` optional after 01-07M, and no global Memory record-version upgrade, old-record backfill or read-time migration is introduced
...
- cross-file impact scan maps H/J/K/M and current tests as downstream work without editing them
...
- Thin Slice owner contains one unambiguous authority, canonical envelope/hash, pattern/vector, final outcome matrix, exact-copy chain, staged H/J/K/M migration and compatibility/ABA/nonclaim ruling
...
new_dependencies: `YES / 01-07M CORE CONTRACT CLOSURE` — staged migration adds one ownership-isolated Packet after the 01-07K/01-07L common barrier and changes the target denominator from 28 to 29. Any further monotonic/HMAC/global/external/migration requirement still blocks this Packet and requires another denominator decision.
...
rollback: Close before merge or use a normal revert PR and re-block 01-07D–01-07M plus 01-08/01-08A. Never reset, force-push, delete data, backfill or invent a migration rollback for this contract-only change.
...
- outcome, staged H/J/K/M migration and exact-copy propagation matrices
...
| `OSV-D01` | Denial of Service | staged missing/malformed version → Presentation | `MITIGATE / BOUNDED` | H expands representation, J fail-closes before Observation, K produces and M contracts; no fallback, retry or second query |
...
<task type="auto">
  ...
  <files>docs/implementation/e2e01-thin-slice-implementation-spec.md</files>
  <action>
把最终FOUND/NOT_FOUND_OR_NOT_ACCESSIBLE/SYSTEM_FAILURE的closed field与Observation/Manifest矩阵、Result→Observation→Manifest exact-copy链、fallback denylist、旧record/global Memory非影响及H/J/K/M下游ownership写入同一owner。必须同时定义H additive expand、J fail-closed acceptance、K producer、M Core contract closure的green migration；不要修改任何consumer文件。
  </action>
  <verify>
    <automated>git diff --check 3f0753f7bef87fc02f314e28fe8b07860a819701...HEAD
rg -n 'FOUND|NOT_FOUND_OR_NOT_ACCESSIBLE|SYSTEM_FAILURE|source_version|byte-for-byte|fallback|stored_at|fixture_version|OrderObservation|ContextManifest|additive|01-07H|01-07J|01-07K|01-07M|rollback' docs/implementation/e2e01-thin-slice-implementation-spec.md >/dev/null
uv run pytest</automated>
人工核对外部不存在/越权仍不可区分、corruption不伪装成not-found、无第二次query或授权扩大。
  </verify>
  <done>
三类outcome、H/J/K/M green migration、exact-copy和fail-closed边界唯一；global Memory/schema/migration/Agent-visible contract保持不变。
  </done>
</task>
...
rg -n \
  'mock-order-source-version|source_version|sha256|861c136b|FOUND|NOT_FOUND_OR_NOT_ACCESSIBLE|SYSTEM_FAILURE|byte-for-byte|fallback|ABA|monotonic|additive|01-07H|01-07J|01-07K|01-07M' \
  "$owned_file"
...
5. C/G串行merge后才记录共同barrier并签发01-07D/01-07H；01-08必须等待K/L共同barrier后的01-07M Core contract closure。
