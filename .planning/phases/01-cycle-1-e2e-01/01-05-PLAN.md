...
> 本 Plan 只映射 exact base `c35687d...`、branch `codex/e2e01-w2-runtime` 与 Draft PR #28；不得在 01-04H 之后改写 `base_sha`、branch、Worktree 或把它复用为 replacement Packet。01-04H reviewed merge 后，Integrator 必须通过新的 single-writer planning PR 签发 `01-05R`，固化当时 exact integration SHA、新 branch / Worktree、原 14-file ownership 与 Runtime review findings；在该新 Plan 合并前不得开始 replacement 写入。
