...
must_haves:
  truths:
    ...
    - "The provider-visible get_order.order_id argument is rebuilt from the current normalized InputBinding, and GetOrderQuery uses customer_id=trusted plus order_id=bound; Provider-supplied replacement values are never dispatched."
...
Integrator 已预建独立 execution Worktree。Executor只能写 `<task_packet>.owned_files` 的精确 14 文件；所有引用文件均为只读输入。三个 Task各自执行 RED → GREEN → 可选 REFACTOR，RED/GREEN证据与提交顺序必须进入 handoff。
</execution_context>
...
- `revalidate_next_move` rebuilds the provider-visible `get_order.order_id` from the current normalized binding, carries `validated_task_state_version=1` for the initial path, and never copies a Provider-supplied trusted field.
- Gateway compares the exact canonical tool name, registry version/snapshot, closed argument schema, binding refs, READ effect and current Task/RequestUnit status/version. It uses existing `GateReasonCode` members only; no duplicate strings or new enum values.
...
| foreign or nonexistent | bounded identical not-found stop from existing enum | `BLOCKED/v2` | `1 / 1 / 1 / 0`; no presentation call |
...
| `RT-T01` | HIGH | Tampering | Provider candidate → InputBinding | `MITIGATE / BLOCK` | current-message normalized binding replaces provider-visible `get_order.order_id`; directed foreign/nonexistent replacement tests assert zero unauthorized dispatch |
...
<task type="auto" tdd="true">
  ...
  <action>
RED: Create the two allowlisted tests before the source modules. Cover one valid current-message ADD_GOAL; source/authority/candidate-version/trusted-field rejection; normalized InputBinding; provider-visible `get_order.order_id` replacement; owner/current Task and RequestUnit revalidation; exact immutable snapshot/name/schema/READ effect/binding checks; unknown tool; inactive state; stale current version 2 against validated version 1; and zero ToolCall authorization on every reject. Assert that `SafeOrderProjection.order_number` never appears in the request-processing or Gateway input shape. Run the focused command and retain the expected failure caused by absent Runtime behavior, not malformed test syntax.

GREEN: Implement `InitialRequestDecision`, `RevalidatedNextMove`, `validate_and_reduce_initial_request`, `revalidate_next_move` and `evaluate_control_gateway` exactly as described in `<interfaces>`. Core performs no I/O and imports no Application module. Use existing frozen models and enum members, generate no identity, accept no ordinary dict bypass, and do not introduce capability classification or RequestUnit Tool allowlists. The initial decision is ACTIVE/v1; rebuilt provider-visible `get_order.order_id` comes only from the normalized current-message binding; the stale case rejects with existing `STATE_VERSION_MISMATCH`. Do not map the downstream `SafeOrderProjection.order_number` field back into Tool arguments.
...
REFACTOR: Remove duplicated comparisons only when behavior tests remain green. Preserve explicit fail-closed checks rather than collapsing them into truthy/falsy generic validation. Commit RED before GREEN with `test(01-05): define request and gateway behavior` followed by `feat(01-05): implement request and gateway decisions`; use an optional `refactor(01-05): simplify pure runtime decisions` only if source changes.
  ...
  <acceptance_criteria>
    ...
    - the approved `get_order.order_id` equals the current normalized InputBinding even when Provider proposes a replacement
...
REFACTOR: Extract private record/Trace builders only inside the two source files; do not create additional files or shared Ports. Keep each persisted event and conditional result check explicit. Commit `test(01-05): define agent run and recovery trajectories`, then `feat(01-05): implement runtime orchestration and recovery`; optional refactor must preserve focused/full green.
  ...
  <acceptance_criteria>
    ...
    - foreign and nonexistent paths are user-visible identical and skip Observation/presentation
    - unknown and stale paths produce zero ToolCall/read/Observation with exact v2/v3 terminal states
...
4. From repository root run the canonical full gate:
...
8. Apply/rebase the published feature onto the latest official integration parent and rerun focused/full gates. Compute normalized paths for `git diff --name-only <overlay-parent>...<overlay-head>` and prove exact set equality with the same 14 `owned_files`; pre-existing sibling files remain in the overlay parent and outside this diff. Obtain independent correctness/security/contract/test-gap review against the resulting exact overlay head.
